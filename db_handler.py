...
class Handler:
    def __init__(self, db_file: str) -> None:
        ...
        (
            ...
            self.active,
        ) = "uid name query url added_timestamp active".split()
        ...
        article_init = f"""CREATE TABLE IF NOT EXISTS {self.article_table} ({self.user_id} INTEGER NOT NULL, {self.keyword} TEXT, {self.url} TEXT NOT NULL, {self.added_timestamp} TEXT, {self.active} INTEGER, UNIQUE({self.user_id}, {self.url}))"""
    ...
    def get_links(self, id: int, keyword: str) -> bool:
        sql = f"""
        SELECT * FROM {self.article_table} WHERE {self.user_id} = {id} AND {self.keyword} = '{keyword}'
        """
        print(sql)
        return self._get(sql)

    def add_links(self, id: int, keyword: str, urls: list) -> bool:
        sql = f"""
        INSERT OR IGNORE INTO {self.article_table}({self.user_id}, {self.keyword}, {self.url}, {self.added_timestamp}, {self.active}) VALUES(?,?,?,?,?)
        """
        data = [(id, keyword, url, datetime.now(), 1) for url in urls]
        print(data)
...
if __name__ == "__main__":
    ...
    add_link = handler.add_links(id=3239, keyword="삼성전자", urls=urls)
